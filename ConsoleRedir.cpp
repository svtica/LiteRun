...
bool CreateIOPipesInService(Settings& settings, LPCWSTR caller, DWORD pid)
{
	...
	// Create StdOut pipe
	pipeName = StrFormat(L"\\\\.\\pipe\\LiteRunOut%s%u", caller, pid);
	...
	if(BAD_HANDLE(settings.hStdOut))
		Log(StrFormat(L"LiteRun failed to create pipe %s.", pipeName), gle);

	// Create StdError pipe
	pipeName = StrFormat(L"\\\\.\\pipe\\LiteRunErr%s%u", caller, pid);
	...
	// Create StdIn pipe
	pipeName = StrFormat(L"\\\\.\\pipe\\LiteRunIn%s%u", caller, pid);
...
BOOL ConnectToRemotePipes(ListenParam* pLP, DWORD dwRetryCount, DWORD dwRetryTimeOut)
{
	...
	CString remoteOutPipeName = StrFormat(L"\\\\%s\\pipe\\LiteRunOut%s%u", pLP->remoteServer, pLP->machineName, pLP->pid);
	CString remoteErrPipeName = StrFormat(L"\\\\%s\\pipe\\LiteRunErr%s%u", pLP->remoteServer, pLP->machineName, pLP->pid);
	CString remoteInPipeName = StrFormat(L"\\\\%s\\pipe\\LiteRunIn%s%u", pLP->remoteServer, pLP->machineName, pLP->pid);
