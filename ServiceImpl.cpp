...
void __cdecl StopServiceAsync(void*)
{
	while(false == gbStop)
	{
		...
		if(0 == gInProcessRequests)
		{
			...
			if(0 == gInProcessRequests)
			{
				Log(L"LiteRun service stopping (async)", false);
...
void WINAPI ServiceControlHandler(DWORD dwControl)
{
	...
	case SERVICE_CONTROL_STOP:
		if(0 == gInProcessRequests)
		{
			...
			if(NULL != ghService)
			{
				Log(L"LiteRun service stopping", false);
				gServiceStatus.dwCurrentState = SERVICE_STOPPED;
				SetServiceStatus(ghService, &gServiceStatus);
			}
		}
		...
		{
			Log(L"LiteRun received request to stop, but still have active requests.  Will stop later.", false);
...
VOID WINAPI ServiceMain(DWORD dwArgc, LPTSTR* lpszArgv)
{
	...
	Log(L"LiteRun ServiceMain starting.", false);
	...
	{
		ghService = RegisterServiceCtrlHandler(L"LiteRun", ServiceControlHandler);
		if(0 == ghService)
		{
			Log(L"RegisterServiceCtrlHandler failed in LiteRun", GetLastError());
			return;
		}
		...
		if(FALSE == b)
			Log(L"Failed to signal LiteRun running.", GetLastError());
	}

	Log(L"LiteRun service running.", false);
	...
	{ 
		...
		{
			CString msg;
			msg.Format(L"LiteRun failed to create pipe %s.", pipename);
		...
		{
			CString msg;
			msg.Format(L"LiteRun created pipe %s.", pipename);
			Log(msg, false);
		}
	...
	Log(L"LiteRun exiting loop.", false);
...
DWORD StartLocalService(CCmdLineParser& cmdParser)
{
	...
	{
		{ const_cast<LPWSTR>(L"LiteRun"), ServiceMain }, //If the service is installed with the SERVICE_WIN32_OWN_PROCESS service type, this member is ignored, but cannot be NULL. This member can be an empty string ("").
		{ NULL, NULL }
	};
	...
	if(false == gbDevOnlyDebug)
	{
		...
		{
			gbODS = true;
			Log(L"LiteRun failed to start ServiceCtrlDispatcher.", GetLastError());
			return GetLastError();
		}
	}
	else
	...
	//still hangs around (ie a long list of LiteRun-xx-yy in services.msc), so here we'll take an additional step and try to delete ourself
